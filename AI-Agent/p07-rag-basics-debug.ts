...
function previewText(text: string, limit: number = 60): string {
...
function describeNonZeroVector(vec: number[]): string[] {
  return vocabulary
    .map((term, i) => ({ term, value: vec[i] ?? 0 }))
...
function naiveVectorize(text: string): number[] {
  ...
  // 统计每个词在当前文本中的出现次数
  const freqMap = getWordFrequency(words);
...
class DocumentStore {
  ...
  addDocument(
    ...
    newChunks.forEach((chunk, i) => {
      console.log(
        `  chunk${i + 1} => id=${chunk.id} | 预览=${previewText(chunk.content)}`,
      );
    });
  ...
  private rebuildVocabularyAndVectorize(): void {
    ...
    console.log(
      `[词汇表已重建] 总词数=${vocabulary.length} | 前20项=${vocabulary
        .slice(0, 20)
        .join(", ")}`,
    );
    ...
    for (const chunk of this.chunks) {
      ...
      const words = tokenize(chunk.content);
      console.log(`  [向量化 chunk] id=${chunk.id}`);
      console.log(`    分词=${words.join(" | ")}`);
      console.log(
        `    非零向量项=${describeNonZeroVector(chunk.vector).join(", ") || "(空)"}`,
      );
    }
  }
  ...
  search(query: string, topK: number = 3): Chunk[] {
    ...
    const queryNonZeroTerms = describeNonZeroVector(queryVec);

    console.log(`\n[开始检索] query=${query}`);
    console.log(`  查询分词=${queryWords.join(" | ") || "(空)"}`);
    console.log(
      `  查询非零向量项=${queryNonZeroTerms.join(", ") || "(空，说明没有和词汇表匹配上的词)"}`,
    );
    ...
    scored.forEach(({ chunk, score }) => {
      console.log(
        `    score=${score.toFixed(6)} | ${chunk.id} | ${previewText(chunk.content)}`,
      );
    });
    ...
    if (topChunks.length === 0) {
      console.log("  [检索结果] 没有任何 chunk 得分大于 0");
    } else {
      console.log("  [Top-K 结果]");
      topChunks.forEach(({ chunk, score }, i) => {
        console.log(
          `    top${i + 1} score=${score.toFixed(6)} | ${chunk.id} | ${previewText(chunk.content)}`,
        );
      });
    }

    return topChunks.map(({ chunk }) => chunk);
  }
